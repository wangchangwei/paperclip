...
export function StatusCards() {
  ...
  const refreshMutation = useMutation({
    ...
    onError: (err) => setActionError(err instanceof Error ? err.message : t("statusCards.refreshError")),
  });
  const recompileMutation = useMutation({
    mutationFn: (id: string) => statusCardsApi.recompile(id),
    onMutate: () => setActionError(null),
    onSuccess: () => invalidateLists(),
    onError: (err) => setActionError(err instanceof Error ? err.message : t("statusCards.runError")),
  });
  const archiveMutation = useMutation({
    mutationFn: (id: string) => statusCardsApi.patch(id, { archived: true }),
    onMutate: () => setActionError(null),
    onSuccess: () => invalidateLists(),
    onError: (err) => setActionError(err instanceof Error ? err.message : t("statusCards.archiveError")),
  });
  const restoreMutation = useMutation({
    mutationFn: (id: string) => statusCardsApi.patch(id, { archived: false }),
    onMutate: () => setActionError(null),
    onSuccess: () => invalidateLists(),
    onError: (err) => setActionError(err instanceof Error ? err.message : t("statusCards.restoreError")),
  });
  ...
  return (
    <div className="mx-auto max-w-6xl space-y-5">
      ...
      {actionError ? <InlineBanner tone="warning" title={t("common.headsUp")}>{actionError}</InlineBanner> : null}
      ...
      {activeQuery.isLoading ? (
        ...
        <InlineBanner tone="danger" title={t("statusCards.loadFailed")}>
          {activeQuery.error instanceof Error ? activeQuery.error.message : t("statusCards.tryAgain")}
        </InlineBanner>
      ) : activeCards.length === 0 ? (
        <EmptyState
          icon={FlaskConical}
          title={t("statusCards.emptyTitle")}
          message={t("statusCards.emptyMessage")}
