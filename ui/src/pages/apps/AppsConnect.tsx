        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl font-bold tracking-tight">
              {appIdentity ? t("appsConnect.connectApp", { name: appIdentity.name }) : t("appsConnect.connectAnApp")}
            </h1>
...
function GalleryStep({
  ...
  const continueWithLink = () => {
    ...
    if (!next) {
      setLinkError(t("appsConnect.pasteFullLink"));
      return;
    }
    setLinkError(null);
    onUseLink(next);
    ...
    <div className="space-y-6">
      <div className="relative">
        ...
        <Input
          ...
          placeholder={t("appsConnect.searchApps")}
          className="h-11 pl-9"
        />
      </div>
            ...
            >
              ...
              <div className="mt-3 text-xs font-semibold text-foreground">
                {unavailable ? (
                  ...
                  <span>{t("appsConnect.connectArrow")}</span>
                )}
              </div>
            </button>
          );
      ...
      >
        <div>
          <div className="flex items-center gap-2 text-sm font-semibold text-foreground">
            ...
            {zapierSource ? t("appsConnect.connectZapier") : byo ? t("appsConnect.connectYourOwn") : t("appsConnect.connectWithLink")}
          </div>
          <p className="mt-1 text-xs text-muted-foreground">
            {zapierSource
              ? t("appsConnect.zapierUrlHint")
              : byo
              ? t("appsConnect.pasteMcpUrl")
              : t("appsConnect.pasteSetupLink")}
          </p>
          {!zapierSource && (
            <p className="mt-1 text-xs text-muted-foreground">
            ...
            <div className="mt-3 flex items-center justify-between gap-3 rounded-lg border border-border bg-muted/40 px-3 py-2">
              ...
              >
                {matchedEntry.availability?.available === false
                  ? t("appsConnect.notAvailable")
                  : matchedEntry.slug === "zapier"
                    ? t("common.continue")
                    : t("appsConnect.useAppName", { name: matchedEntry.name })}
              </Button>
            </div>
          )}
        </div>
        <div className="flex min-w-0 flex-col gap-2 sm:min-w-(--sz-360px)">
          <div className="flex gap-2">
            <Input
              ...
              placeholder={zapierSource ? "https://mcp.zapier.com/api/v1/connect?token=…" : t("appsConnect.linkPlaceholder")}
              className="h-10"
            />
      ...
      <div className="border-t border-border pt-5">
        ...
        <p className="mt-1 text-xs text-muted-foreground">
          {t("appsConnect.moreWaysDescription")}
        </p>
        <div className="mt-3 flex flex-col gap-2">
          <ConnectMethodRow
            icon={TerminalSquare}
            title={t("appsConnect.runYourOwn")}
            description={t("appsConnect.runYourOwnDescription")}
            onClick={onRunYourOwn}
          />
          <ConnectMethodRow
            icon={ClipboardPaste}
            title={t("appsConnect.pasteAConfig")}
            description={t("appsConnect.pasteConfigDescription")}
  ...
  return (
    <div className="mx-auto max-w-xl rounded-2xl border border-border bg-card p-8">
      ...
      <div className="mt-8 space-y-6">
        ...
        <div>
          <label className="text-sm font-medium text-foreground">{t("appsConnect.doesItNeedKey")}</label>
          ...
          <p className="mt-2 text-xs text-muted-foreground">
            {needsKey
              ? t("appsConnect.pasteTheKey")
              : t("appsConnect.mostAppsJustWork")}
          </p>
        </div>
        ...
        {needsKey && (
          <div className="space-y-4">
            <div>
              <label className="text-sm font-medium text-foreground">{t("appsConnect.appKey")}</label>
              <Input
                type="password"
                autoComplete="off"
                value={keyValue}
    ...
    return (
      <div className="mx-auto max-w-xl rounded-2xl border border-border bg-card p-8">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-lg font-bold tracking-tight sm:text-xl">{t("appsConnect.connectGoogleSheets")}</h2>
        ...
        <div className="mt-8 space-y-6">
          ...
          {robotEmail ? (
            <div>
              <label className="text-sm font-medium text-foreground">{t("appsConnect.shareEachSheet")}</label>
              ...
              <p className="mt-2 text-xs text-muted-foreground">
                {t("appsConnect.googleSheetsShareInstructions")}
              </p>
            </div>
          ) : (
            <div className="rounded-lg bg-muted/50 p-4 text-sm text-muted-foreground">
              {t("appsConnect.googleSheetsNotAvailable")}
            </div>
          )}

          <div>
            <label className="text-sm font-medium text-foreground">{t("appsConnect.pasteSheetLinks")}</label>
            <Textarea
  ...
  return (
    <div className="mx-auto max-w-xl rounded-2xl border border-border bg-card p-8">
      <div className="flex items-center gap-3">
        <AppLogo name={entry.name} logoUrl={entry.branding.logoUrl} size={48} />
        <div>
          <h2 className="text-xl font-bold tracking-tight">{t("appsConnect.connectApp", { name: entry.name })}</h2>
          <p className="text-sm text-muted-foreground">{copy.short}</p>
        </div>
      </div>
  ...
  return (
    <div className="space-y-5">
      <div className="flex items-start gap-3">
        ...
        <div>
          <div className="text-lg font-bold text-foreground">
            {t("appsConnect.connectedToApp", { appName, total, action: total === 1 ? "action" : "actions" })}
          </div>
          <div className="text-sm text-muted-foreground">
            {t("appsConnect.readOnlyOn")}
          </div>
        </div>
      </div>

      <ActionGroup
        title={t("appsConnect.readOnly")}
        hint={t("appsConnect.readOnlyHint")}
        actions={readOnly}
        enabled={enabled}
        onToggle={onToggle}
        bulkLabel={t("appsConnect.turnAllOff")}
        onBulk={() => onBulk(readOnly.map((a) => a.catalogEntryId), false)}
        askFirstLevels={askFirstLevels}
      />

      <ActionGroup
        title={t("appsConnect.canMakeChanges")}
        hint={t("appsConnect.canMakeChangesHint")}
        actions={canMakeChanges}
        enabled={enabled}
        onToggle={onToggle}
        bulkLabel={t("appsConnect.turnAllOn")}
      ...
      <div className="flex items-center justify-between pt-1">
        ...
        <div className="flex items-center gap-3">
          <span className="hidden text-xs text-muted-foreground sm:inline">
            {t("appsConnect.newActionsStartOff", { appName })}
          </span>
          <Button onClick={onContinue} disabled={enabledCount === 0}>
            {t("appsConnect.continueWithActions", { enabledCount, action: enabledCount === 1 ? "action" : "actions" })}
          </Button>
        </div>
      </div>
    </div>
  ...
  return (
    <div className="mx-auto max-w-xl">
      <div className="rounded-2xl border border-border bg-card p-8">
        <h2 className="text-xl font-bold tracking-tight">{t("appsConnect.whoCanUse", { appName })}</h2>
        <p className="mt-1 text-sm text-muted-foreground">{t("appsConnect.changeLater")}</p>
        ...
        <div className="mt-6 space-y-3">
          ...
          >
            ...
            <div>
              <div className="flex items-center gap-2">
                <span className="font-bold text-foreground">{t("appsConnect.allAgents")}</span>
                <span className="rounded-full bg-foreground px-2 py-0.5 text-(length:--text-nano) font-bold text-background">
                  {t("appsConnect.recommended")}
                </span>
              </div>
              <p className="mt-1 text-xs text-muted-foreground">
                {t("appsConnect.anyoneCanUseApp", { appName })}
              </p>
            </div>
          </button>
          ...
          >
            ...
            <div className="flex-1">
              <span className="font-semibold text-foreground">{t("appsConnect.onlySpecificAgents")}</span>
              <p className="mt-1 text-xs text-muted-foreground">{t("appsConnect.tickAgentsWhoCanUse", { appName })}</p>
            </div>
          </button>
      ...
      <div className="mt-6 flex items-center justify-between">
        ...
        <Button onClick={onContinue} disabled={!canFinish}>
          {t("appsConnect.continueToInstall")}
        </Button>
      </div>
    </div>
  );
...
export function InstallStep({
  ...
  const extendingLabel = extendingAgentIds.length === 1
    ? agents.find((agent) => agent.id === extendingAgentIds[0])?.name ?? t("appsConnect.oneAgent")
    : t("appsConnect.numAgents", { count: extendingAgentIds.length });

  return (
    <div className="mx-auto max-w-xl">
      <div className="rounded-2xl border border-border bg-card p-8">
        <h2 className="text-xl font-bold tracking-tight">{t("appsConnect.installTools", { appName })}</h2>
        <p className="mt-1 text-sm text-muted-foreground">
          {t("appsConnect.accessVsInstall")}
        </p>
        ...
        <div className="mt-6 space-y-3">
          ...
          >
            ...
            <div>
              <span className="font-semibold text-foreground">{t("appsConnect.notYet")}</span>
              <p className="mt-1 text-xs text-muted-foreground">
                {t("appsConnect.keepPermittedOnly", { appName })}
              </p>
            </div>
          </button>
          ...
          >
            ...
            <div className="flex-1">
              <span className="font-semibold text-foreground">{t("appsConnect.specificAgents")}</span>
              <p className="mt-1 text-xs text-muted-foreground">{t("appsConnect.tickAgentsLoadEveryRun", { appName })}</p>
            </div>
          </button>
          ...
          >
            ...
            <div>
              <span className="font-semibold text-foreground">{t("appsConnect.allAgents")}</span>
      ...
      <div className="mt-6 flex items-center justify-between">
        ...
        <Button onClick={onFinish} disabled={submitting || !canFinish}>
          {submitting && <Loader2 className="mr-2 h-4 w-4 animate-spin" />}
          {submitting ? t("appsConnect.finishing") : t("appsConnect.finishSetup")}
        </Button>
      </div>
    </div>
  );
...
function SuccessStep({
  ...
  const installSummary = installMode === "all"
    ? t("appsConnect.installedAllAgents")
    : installMode === "specific"
      ? t("appsConnect.numAgentsInstalled", { count: installCount })
      : t("appsConnect.permittedOnly");
  return (
    <div className="mx-auto max-w-md py-10 text-center">
      ...
      <div className="mt-6 flex items-center justify-center gap-2">
        ...
        <h2 className="text-2xl font-bold tracking-tight">{t("appsConnect.appReady", { appName })}</h2>
      </div>
      <p className="mt-2 text-sm text-muted-foreground">
        {installMode === "none"
          ? t("appsConnect.agentsCanUseAfterInstall")
          : t("appsConnect.installedAgentsNextRun")}
      </p>
      <p className="mt-1 text-xs text-muted-foreground">
        {t("appsConnect.actionsOnByAccess", { enabledCount, access: access === "all" ? t("appsConnect.allAgentsCanUse") : t("appsConnect.specificAgentsCanUse"), installSummary })}
      </p>
      <div className="mt-8">
