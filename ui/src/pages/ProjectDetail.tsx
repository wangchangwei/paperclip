    ...
    >
      <PopoverTrigger asChild>
        <button
          ...
          aria-label={t("projects.changeProjectIconAndColor")}
        >
      ...
      <PopoverContent className="w-72 p-3" align="start">
        {/* Icon search + grid */}
        <p className="text-xs font-medium text-muted-foreground mb-2">{t("projects.icon")}</p>
        <Input
        ...
        <div className="grid grid-cols-7 gap-1 max-h-40 overflow-y-auto">
          ...
          {filteredIcons.length === 0 && (
            <p className="col-span-7 text-xs text-muted-foreground text-center py-2">{t("projects.noIconsMatch")}</p>
          )}
        </div>

        {/* Color swatches */}
        <div className="mt-3 border-t border-border pt-3">
          <p className="text-xs font-medium text-muted-foreground mb-2">{t("projects.color")}</p>
          <div className="grid grid-cols-5 gap-1.5">
            ...
            <button
              ...
              aria-label={t("projects.resetToNeutralGray")}
              title={t("projects.neutralDefault")}
            >
            ...
            {PROJECT_COLORS.map((swatch) => (
              <button
                ...
                aria-label={t("projects.selectColor", { swatch })}
              />
            ))}
          </div>
        </div>
  ...
  return (
    <div className="space-y-6">
      {showLeftProjectNotice ? (
        <div className="flex items-center gap-3 border border-yellow-300/35 bg-yellow-300/10 px-3 py-2 text-sm text-yellow-900 dark:text-yellow-100">
          ...
          <button
            ...
            aria-label={t("projects.dismissLeftNotice")}
      ...
      <Tabs value={activeTab ?? "list"} onValueChange={(value) => handleTabChange(value as ProjectTab)}>
        <PageTabBar
          items={[
            { value: "list", label: t("projects.tasksTab") },
            { value: "overview", label: t("projects.overviewTab") },
            ...(project.managedByPlugin ? [{ value: "plugin-operations", label: t("projects.pluginOperationsTab") }] : []),
            ...(showWorkspacesTab ? [{ value: "workspaces", label: t("projects.workspacesTab") }] : []),
            { value: "configuration", label: t("projects.configurationTab") },
            { value: "budget", label: t("projects.budgetTab") },
