...
export function ProfileSettings() {
  ...
  useEffect(() => {
    setBreadcrumbs([
      { label: t("profileSettings.settings"), href: "/company/settings" },
      { label: t("profileSettings.instanceSettings"), href: "/company/settings/instance/general" },
      { label: t("profileSettings.profile") },
    ]);
  }, [setBreadcrumbs, t]);
  ...
  function resolveProfileName() {
    return name.trim() || sessionQuery.data?.user.name || t("profileSettings.boardFallback");
  }
  ...
  const updateMutation = useMutation({
    ...
    onError: (error) => {
      setActionError(error instanceof Error ? error.message : t("profileSettings.updateFailed"));
    },
  });
  ...
  const uploadAvatarMutation = useMutation({
    mutationFn: async (file: File) => {
      if (!selectedCompanyId) {
        throw new Error(t("profileSettings.uploadNoCompany"));
      }
    ...
    onError: (error) => {
      setActionError(error instanceof Error ? error.message : t("profileSettings.uploadFailed"));
    },
  });
  ...
  const removeAvatarMutation = useMutation({
    ...
    onError: (error) => {
      setActionError(error instanceof Error ? error.message : t("profileSettings.removeFailed"));
    },
  });
  ...
  if (sessionQuery.error || !sessionQuery.data) {
    return (
      <div className="text-sm text-destructive">
        {sessionQuery.error instanceof Error ? sessionQuery.error.message : t("profileSettings.loadFailed")}
      </div>
    );
  }

  const currentName = name.trim() || sessionQuery.data.user.name || t("profileSettings.boardFallback");
  ...
  const uploadHint = selectedCompany
    ? t("profileSettings.uploadHintStored", { company: selectedCompany.name })
    ...
    <div className="max-w-4xl space-y-6">
      <div className="space-y-2">
        <div className="flex items-center gap-2">
          ...
          <h1 className="text-lg font-semibold">{t("profileSettings.profile")}</h1>
        </div>
            ...
            <div className="flex flex-wrap items-end gap-5 rounded-(--rad-24) border border-border/70 bg-background/92 p-5 shadow-(--shadow-extract-18) backdrop-blur-sm">
              ...
              <div className="min-w-0 flex-1 space-y-2 pb-1">
                ...
                <p className="max-w-2xl text-sm leading-6 text-muted-foreground">
                  {t("profileSettings.avatarHelp", { hint: uploadHint })}
                </p>
              </div>
            </div>
          </div>
        ...
        >
          <div className="space-y-2">
            ...
            <Input
              ...
              maxLength={120}
              placeholder={t("profileSettings.boardFallback")}
            />
            <p className="text-xs text-muted-foreground">
              {t("profileSettings.displayNameHelp")}
            </p>
          </div>
